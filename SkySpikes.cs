...
public class SkySpikes : MonoBehaviour {
    ...
    private IEnumerator Up()
    {
        while (true)
        {
            son_Transform.position = Vector3.Lerp(son_Transform.position, targetPos, Time.deltaTime * 20);
            yield return null;
        }
    }
    ...
    private IEnumerator Down()
    {
        while (true)
        {
            son_Transform.position = Vector3.Lerp(son_Transform.position, normalPos, Time.deltaTime * 20);
            yield return null;
        }
    }
}
